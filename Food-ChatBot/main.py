...
async def handle_request(request: Request):
    #retrive json data from request
    payload = await request.json()
    
    #Extract the neccessary information from the payload
    #based on structure of webhookrequest from dialogflow
    intent = payload['queryResult']['intent']['displayName']
    parameter = payload['queryResult']['parameters']
    output_context = payload['queryResult']['outputContexts']
    session_id = genatic_helper.extract_session_id(output_context[0]['name'])
    
    
    intent_handler_dict = {
        'order.add - context: ongoing-order':add_to_order,
        'order.reomve - context: ongoing-order':remove_from_order,
...
def add_to_order(parameter: dict,session_id:str):
    ...
    if len(food_items) != len(quantities):
        ...
        order_str = genatic_helper.get_str_from_food_dict(inprogress_orders[session_id])    
        fulfillment_text = f"So far you have order {order_str}. Do you need any thing else?"
...
def remove_from_order(parameter: dict, session_id: str):
    ...
    if len(current_order.keys()) == 0:
        ...
        order_str = genatic_helper.get_str_from_food_dict(current_order)
...
def track_order(parameter: dict, session_id: str):
    order_id = parameter['number']
    status = db_helper.get_order_status(order_id)   
    
    if status:
        fulfillment_text = f"The order status for order id: {int(order_id)} is: {status}"
    else:
